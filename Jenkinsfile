    ...
    stages {
        ...
        stage('Continuous Integration') {
            steps {
                ...
                sh 'sleep 10'
                sh 'docker exec mybank_backend_container php bin/console doctrine:migrations:migrate --env=test --no-interaction'
                sh 'docker exec mybank_backend_container php bin/console doctrine:fixtures:load --env=test --no-interaction'
                sh 'docker exec mybank_backend_container php bin/console lexik:jwt:generate-keypair --env=test'
                sh 'docker exec mybank_backend_container php bin/phpunit'
